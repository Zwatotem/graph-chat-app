...
namespace GraphChatApp
{
		...
		public async void requestCreateNewUser(object sender, UserRegisteredEventArgs args)
		{
			...
			bool response = await Task.Run(() =>
			{
				lock (this)
				{
					...
					if (rsp)
					{
						var addedUser = chatSystem.addNewUser(proposedName);
	...
	public class SuccessfullyLoggededEventArgs : EventArgs
	{
		...
		public SuccessfullyLoggededEventArgs(User user)
		{
			loggedUser = user;
		}
	}
	...
	public class SuccessfullyRegisteredEventArgs : EventArgs
	{
		...
		public User RegisteredUser { get => registeredUser; set => registeredUser = value; }
		public SuccessfullyRegisteredEventArgs(User user)
		{
			registeredUser = user;
		}
	}
