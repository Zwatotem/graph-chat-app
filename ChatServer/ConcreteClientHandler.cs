...
namespace ChatServer
{
    ...
    public class ConcreteClientHandler : IClientHandler
    {
        ...
        private IRequestHandlerCreator requestHandlerCreator; //contains the factory method necessary for creating instances of IRequestHandler
        private int requestHeaderLength; //length of request's header - may vary depending on the implementation

        public string HandledUserName { get => handledUserName; set => handledUserName = value; }
        ...
        public ConcreteClientHandler(IServerChatSystem chatSystem, Socket socket, List<IClientHandler> allHandlers, int headerLength)
        {
            ...
            this.socketFacade = new ConcreteIOSocketFacade(socket); //socket is enclosed withing the facade as to simplify its usage
        ...
        private void listen()
        {
            ...
            while (working)
            {
                ...
                try
                {
                    ...
                    int messageLength = BitConverter.ToInt32(headerBytes, 1); //decode how long the message to receive is
                ...
                IRequestHandler requestHandler = requestHandlerCreator.createRequestHandler(typeByte); //using factory method to create request
                ...
                requestHandler.handleRequest(allHandlers, chatSystem, this, messageBytes); //using strategy pattern to handle client request.
