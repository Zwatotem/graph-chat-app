...
$graphingCharacters =	gc((gi $PSCommandPath).DirectoryName + '\graphingCharacters.in')
...
$sendMessageSubset = 	gc((gi $PSCommandPath).DirectoryName + '\sendMessageSubset.in')
...
$client3 = [System.Diagnostics.Process]::Start($startInfo)
Write-Debug "Third instance started"
...
& $sendInput $client1 $registerUser.Replace('user', 'user3')
& $sendInput $client1 $loginUser.Replace('user', 'user1')
Start-Sleep 1
Write-Debug "Logging user 2"
& $sendInput $client2 $loginUser.Replace('user', 'user2')
Start-Sleep 1
Write-Debug "Logging user 3"
& $sendInput $client3 $loginUser.Replace('user', 'user3')
$userList = @"
user2
user3
"@
& $sendInput $client1 $createConversation.Replace('user', $userList)
...
& $sendInput $client2 $sendMessageSubset.Replace('-1', '1').Replace("messageText", $message+'reply')
Start-Sleep 1
Write-Debug "Closing"
& $sendInput $client2 $end
$out = $client2.StandardOutput.ReadToEnd()
Stop-Process $server, $client1, $client2, $client3
$out.Contains('subset of users')
